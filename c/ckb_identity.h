...
int verify_sighash_all(uint8_t *pubkey_hash, uint8_t *sig, uint32_t sig_len,
                       validate_signature_t func, convert_msg_t convert, uint8_t* signing_message_hash) {
  int ret = 0;
...
int verify_via_dl(CkbIdentityType *id, uint8_t *sig, uint32_t sig_len,
                  ...
                  CkbSwappableSignatureInstance *inst, uint8_t* signing_message_hash) {
  int err = 0;
...
int verify_via_exec(CkbIdentityType *id, uint8_t *sig, uint32_t sig_len,
                    uint8_t *preimage, uint32_t preimage_len, uint8_t* signing_message_hash) {
  int err = 0;
  uint8_t hash[BLAKE2B_BLOCK_SIZE];
  ...
  if (err != 0) return err;
  err = ckb_exec_append(&bin_args, signing_message_hash, BLAKE2B_BLOCK_SIZE);
  if (err != 0) return err;
...
int ckb_verify_identity(CkbIdentityType *id, uint8_t *sig, uint32_t sig_size,
                        uint8_t *preimage, uint32_t preimage_size, uint8_t* signing_message_hash) {
